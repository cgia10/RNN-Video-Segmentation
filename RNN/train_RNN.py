...
for epoch in range(1, opt.n_epochs+1):
    ...
    for idx, (features, label) in enumerate(dataloader_train):    # dataloader has length equal to number of videos loaded
        ...
        pred = my_net(features) # 1x11 (1 is the batch size)
        loss = loss_criteria(pred, label) # expects 1x11
        ...
        print("[Epoch %d/%d] [Batch %d/%d] [Loss: %f]" % (epoch, opt.n_epochs, idx, len(dataloader_train), loss.item()))
